...
class TrafficWnd(QMainWindow, Ui_MainWindow):
    ...
    def _init_ui(self):
        ...
        self.IP_lineEdit.setText('192.168.1.200')
    ...
    def login_btn_onclick(self):
        ...
        if self.loginID:
            self.setWindowTitle('Insystlabs (Intelligent Traffic System) Online ')
    ...
    def logout_btn_onclick(self):
        ...
        self.Attach_pushButton.setEnabled(False)
        self.Detach_pushButton.setEnabled(False)
        self.setWindowTitle(" Insystlabs Intelligent Traffic System (OffLine)") 
    ...
    # Start implementing the preview
    ...
    # Smart traffic checkpoint event subscription
        ...
        if(dwAlarmType == EM_EVENT_IVS_TYPE.TRAFFICJUNCTION):
            if(self.row > 499):
                ...
                self.Attach_tableWidget.setHorizontalHeaderLabels(
                    ['Time', 'Event', 'Plate No.', 'Plate Color', 'Vehicle Type',
                    'Vehicle Color'])
                self.row = 0
                # ui update
            ...
            item2 = QTableWidgetItem('(Traffic junction event)')
    ...
    # Realize disconnection reconnection callback function
